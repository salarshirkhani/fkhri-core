(function(){
  function rebuild(root){
    ...
    let line = root.querySelector('.fkhri-ticker-line');
    let firstTrack = root.querySelector('.fkhri-ticker-track');

    if (!line){
      line = document.createElement('div');
      line.className = 'fkhri-ticker-line';
      if (firstTrack) {
    ...
    line.querySelectorAll('.fkhri-ticker-track.__clone').forEach(n => n.remove());
  ...
  function boot(scope){
    (scope || document).querySelectorAll('.fkhri-ticker-carousel').forEach(rebuild);
  }
  ...
  if (window.elementorFrontend && window.elementorFrontend.hooks){
    elementorFrontend.hooks.addAction(
      'frontend/element_ready/fkhri-ticker-carousel.default',

jQuery(function($){
  ...
  function enhanceQty($scope){
    $scope.find('form.cart .quantity').each(function(){
      var $q = $(this);
      if ($q.data('fkhri-enhanced')) return; // فقط یک‌بار
      var $input = $q.find('input.qty');
      if (!$input.length) return;

      $q.data('fkhri-enhanced', true);
      var min = parseFloat($input.attr('min')) || 1;
      var step = parseFloat($input.attr('step')) || 1;
      var max = parseFloat($input.attr('max')) || Infinity;

      $q.addClass('fkhri-qty');
      var $minus = $('<button type="button" class="fkhri-qty__btn" aria-label="کم کردن">−</button>');
      var $plus  = $('<button type="button" class="fkhri-qty__btn" aria-label="زیاد کردن">+</button>');
  ...
  /* کپی لینک اشتراک */
  $(document).on('click', '.fkhri-share__copy', function(){

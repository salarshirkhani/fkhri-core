;(function($){
  ...
  function initSwiperInPanel($panel){
    ...
    const $cont = $wrap.find('.fkhri-tpc__wrap'); // .swiper container
    ...
    const nextEl = $wrap.find('.fkhri-tpc__arrow.next')[0] || null;
    const prevEl = $wrap.find('.fkhri-tpc__arrow.prev')[0] || null;
    const pagEl  = $wrap.find('.fkhri-tpc__dots')[0]         || null;
  ...
  function tickCountdowns($scope){
    const $items = $scope.find('.fkhri-card__countdown[data-countdown]');
  ...
  function init($scope){
    const $root = $scope.find('.fkhri-tpc');
    if(!$root.length) return;

    // تب‌ها
    const $tabs   = $root.find('.fkhri-tpc__tab');
    const $panels = $root.find('.fkhri-tpc__panel');
  ...
  $(window).on('elementor/frontend/init', function(){
    elementorFrontend.hooks.addAction('frontend/element_ready/fkhri-tabbed-product-carousel.default', init);
  });
})(jQuery);

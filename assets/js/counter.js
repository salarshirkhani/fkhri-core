(function(){
  ...
  function boot(scope){
    (scope || document).querySelectorAll('.fkhri-counter').forEach(observe);
  }

  document.addEventListener('DOMContentLoaded', function(){ boot(document); });

  if (window.elementorFrontend && window.elementorFrontend.hooks) {
    elementorFrontend.hooks.addAction('frontend/element_ready/fkhri-counter.default', function($scope){
      boot($scope[0]);
    });
  }
})();

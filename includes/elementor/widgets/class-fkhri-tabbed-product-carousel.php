...
class Fkhri_Tabbed_Product_Carousel extends Widget_Base {
  public function get_name(){ return 'fkhri-tabbed-product-carousel'; }
  public function get_title(){ return 'Fkhri – Tabbed Product Carousel'; }
  public function get_icon(){ return 'eicon-slider-push'; }
  public function get_categories(){ return ['fkhri']; }

  public function get_style_depends(){
    return ['swiper','fkhri-tabbed-carousel'];
  }

  public function get_script_depends(){
    return ['elementor-frontend','swiper','fkhri-tabbed-carousel'];
  }
  ...
  protected function query_featured_timed($s){
    ...
    $args = [
      ...
      'orderby'=>'date','order'=>'DESC',
      'meta_query'=>[
        ['key'=>'_fkhri_featured_enabled','value'=>'yes'],
        ['key'=>'_fkhri_featured_start','value'=>$now,'compare'=>'<=','type'=>'NUMERIC'],
        ['key'=>'_fkhri_featured_end','value'=>$now,'compare'=>'>=','type'=>'NUMERIC'],
      ],
      'tax_query'=>[],
    ];
    if ( 'yes' === ($s['only_instock']??'') ) {
  ...
  protected function product_card($product, $show_countdown=false){
    ...
    $end_ts = $show_countdown ? intval(get_post_meta($product->get_id(), '_fkhri_featured_end', true)) : 0;
    ?>
    <div class="fkhri-card">
      <a class="fkhri-card__thumb" href="<?php echo esc_url(get_permalink($product->get_id())); ?>">
        <?php echo $product->get_image('woocommerce_thumbnail', ['class'=>'fkhri-card__img']); ?>
      </a>
      <div class="fkhri-card__body">
        <h3 class="fkhri-card__title">
          <a href="<?php echo esc_url(get_permalink($product->get_id())); ?>"><?php echo esc_html($product->get_name()); ?></a>
        </h3>
        <div class="fkhri-card__price"><?php echo wp_kses_post($product->get_price_html()); ?></div>

        <?php if ($show_countdown && $end_ts): ?>
          <div class="fkhri-card__countdown" data-countdown="<?php echo esc_attr($end_ts); ?>">
            <span class="d">00</span><b>:</b><span class="h">00</span><b>:</b><span class="m">00</span><b>:</b><span class="s">00</span>
          </div>
        <?php endif; ?>

        <div class="fkhri-card__actions">
          <?php if($product->is_purchasable()){
            $args_btn=[
              'class'=>'fkhri-card__btn add_to_cart_button ajax_add_to_cart',
              'attributes'=>[
            ...
            echo apply_filters('woocommerce_loop_add_to_cart_link',
              sprintf('<a href="%s" data-quantity="1" class="%s" %s>%s</a>',
                ...
                esc_html__('افزودن به سبد خرید','fkhri')
              ), $product, $args_btn);
          } ?>
        </div>
      </div>
  ...
  protected function render(){
    ...
    <div id="<?php echo esc_attr($uid); ?>" class="fkhri-tpc" dir="rtl">
      <!-- Tabs -->
      <div class="fkhri-tpc__tabs" role="tablist">
        <button class="fkhri-tpc__tab is-active" data-tab="all"  role="tab" aria-selected="true"><?php echo esc_html($s['tab_all_title'] ?? 'همه'); ?></button>
        <button class="fkhri-tpc__tab"         data-tab="feat" role="tab" aria-selected="false"><?php echo esc_html($s['tab_feat_title'] ?? 'ویژه'); ?></button>
        <button class="fkhri-tpc__tab"         data-tab="sale" role="tab" aria-selected="false"><?php echo esc_html($s['tab_sale_title'] ?? 'تخفیف‌ها'); ?></button>
      </div>

      <!-- Panels -->
      <div class="fkhri-tpc__panel is-active" data-panel="all">
        <div class="fkhri-tpc__wrap swiper" data-fkhri-swiper='<?php echo wp_json_encode($opts); ?>'>
        ...
        <?php if('yes'===$s['show_arrows']): ?>
          <button class="fkhri-tpc__arrow prev" type="button" aria-label="قبلی"></button>
          <button class="fkhri-tpc__arrow next" type="button" aria-label="بعدی"></button>
        <?php endif; ?>
        <?php if('yes'===$s['show_dots']): ?><div class="fkhri-tpc__dots"></div><?php endif; ?>
      </div>

      <div class="fkhri-tpc__panel" data-panel="feat">
        <div class="fkhri-tpc__wrap swiper" data-fkhri-swiper='<?php echo wp_json_encode($opts); ?>'>
          <div class="swiper-wrapper">
      ...
      <div class="fkhri-tpc__panel" data-panel="sale">
        <div class="fkhri-tpc__wrap swiper" data-fkhri-swiper='<?php echo wp_json_encode($opts); ?>'>
          <div class="swiper-wrapper">
